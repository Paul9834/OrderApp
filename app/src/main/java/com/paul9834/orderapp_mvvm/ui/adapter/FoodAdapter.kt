...
class FoodAdapter (private val context: Context, private val foods:MutableList<ResultsItem>): RecyclerView.Adapter<BaseViewHolder<*>> (){
    ...
    inner class MainViewHolder(itemView: View) :BaseViewHolder<ResultsItem> (itemView) {
        override fun bind(item: ResultsItem, position: Int) {

            Glide.with(context)
                .load(item.thumbnail)
                .apply(RequestOptions.bitmapTransform(RoundedCorners(14)))
                .into(itemView.img_trago)
            itemView.txt_titulo.text = item.title
            itemView.txt_description.text = item.ingredients
